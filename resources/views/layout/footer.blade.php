<!-- Main Footer -->
<footer class="main-footer Rectangle-15">
    <!-- Default to the left -->
    <div class="row mt-5">
       
        <div class="col-lg-1  mb-2 pl-5">
        </div>
        <div class="col-lg-3 col-md-12 mb-2">
            <legend class="MAPA-DE-NAVEGAO ">{{trans('footer.navigation_map')}}</legend>
            <hr class="LineFooter">
            <ul>
                <li class=""><a class="FooterLinks" href="#"> {{trans('footer.know')}}</a></li>
                <li class=""><a class="FooterLinks" href="#"> {{trans('footer.plans')}}</a></li>
                <li><a class="FooterLinks" href="#"> {{trans('footer.im_a_customer')}}</a></li>
                <li><a class="FooterLinks" href="#"> {{trans('footer.register')}}</a></li>
                <li><a class="FooterLinks" href="#"> {{trans('footer.contact')}}</a></li>
            </ul>
        </div>
        <div class="col-lg-4 col-md-12 mb-4">
            <legend class="MAPA-DE-NAVEGAO">{{trans('footer.about')}}</legend>
            <hr class="LineFooter">
            <div>
                <span class="Type-something">{{trans('footer.text_about')}}</span>
            </div>
        </div>
        <div class="col-lg-3 col-md-12 mb-3">
            <legend class="MAPA-DE-NAVEGAO">{{trans('footer.last_of_the_blog')}}</legend>  
            <hr class="LineFooter">
            <ul>
                <li class=""><span class="O-que-seria-Customer" href="#"> {{trans('footer.blog_text_1')}}<br/><span class="text-style-1">{{trans('footer.data_footer')}}</span></span></li>
                <li class=""><span class="O-que-seria-Customer" href="#"> {{trans('footer.blog_text_2')}}<br/><span class="text-style-1">{{trans('footer.data_footer')}}</span></span></li>
                <li><span class="O-que-seria-Customer" href="#"> {{trans('footer.blog_text_3')}}<br/><span class="text-style-1">{{trans('footer.data_footer')}}</span></span></li>
            </ul>
        </div>
    </div>
    <hr class="LineFooter">
    <div class="col-lg-12 col-md-12 mb-12">
        <center>
            <img class="Bitmap" crossorigin="anonymous" src="https://cdn.zeplin.io/59b7ec30a09e2bcccf9bbd10/assets/DE67F334-71AF-499B-987D-B08BCC0B3722.png">
        </center>
    </div>
</footer>